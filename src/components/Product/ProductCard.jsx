...
export default function ProductCard({
  ...
  const isItemExist = wishListProducts.some((item) => item.id === id);
  // console.log(isItemExist);
  return (
    <>
      <div className="product card">
        <div className="images h-80">
          <img
            src={images[0].image}
            alt={id}
          ...
          <div className="overlay absolute bottom-0 left-0 right-0 m-5 flex items-center justify-center gap-2">
            ...
            <button
              className="add-to-cart product-btn primary-btn"
              onClick={() =>
                dispatch(addToCart({ id, title, price: discountPrice, images }))
              }
            >
              <IoCart size={23} />
            </button>
            ...
            {!isItemExist ? (
              <button
                className="love-brn product-btn remove-wish"
                onClick={() =>
                  dispatch(
                    addToWishlist({ id, title, price: discountPrice, images }),
                  )
                }
              >
